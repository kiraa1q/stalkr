    ...
    <div className={`player-card ${open ? 'open' : ''} ${!player.online ? 'offline-card' : ''}`}>
      ...
      <div className="player-header" onClick={() => setOpen(o => !o)}>
        ...
        <div className="player-info">
          ...
          <div className="player-meta">
            <span className="gamemode-tag" style={!player.online ? { opacity: 0.5 } : {}}>
              {player.gamemode}
            </span>
        ...
        <div className="header-right">
          <div className={`online-badge ${player.online ? 'online' : 'offline'}`}>
          ...
          <div className="inv-panel">
              ...
              <div className="armor-col">
                {ARMOR_SLOTS.map(slot => {
                  ...
                  return (
                    <div
                      key={slot}
                      className={`item-slot ${!isEmpty ? 'mat-netherite' : ''}`}
                      data-tip={piece?.name ?? `No ${slot}`}
                    >
                      <img
                        src={ARMOR_ICONS[slot]}
                        style={{
                          width: '42px',
                          height: '42px',
                          imageRendering: 'pixelated',
                          opacity: isEmpty ? 0.12 : 1,
                        }}
                        alt={slot}
                      />
                    </div>
            ...
            <div className="inv-right">
              ...
              <div className="stats-row-mini">
                ...
                <div className="stat-mini">
                  <div className="stat-mini-val">{player.distanceTraveled}km</div>
      ...
      <div className="coords-footer">
        <div className="coords-label">
          ...
          {!player.online && player.lastSeen ? ` · Last seen ${player.lastSeen}` : ''}
        </div>
        <div className="coords-value">
          <div className="coord"><span className="coord-axis">X</span> {player.position.x}</div>
          <div className="coord"><span className="coord-axis">Y</span> {player.position.y}</div>
          <div className="coord"><span className="coord-axis">Z</span> {player.position.z}</div>
        </div>
      </div>
