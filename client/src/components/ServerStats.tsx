...
export default function ServerStatsComponent({ stats }: Props) {
  ...
  const tpsColor = (stats?.tps ?? 0) > 18 ? 'var(--green)' : (stats?.tps ?? 0) > 15 ? 'var(--yellow)' : 'var(--red)';

  return (
    <>
      <div className="section-title">Server</div>
      <div className="server-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '12px' }}>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: tpsColor }}>{stats?.tps ?? '—'}</div>
          <div className="tps-bar-bg" style={{ height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', marginTop: '8px' }}>
            <div 
        ...
        <div className="stat-card" style={{ opacity: 0.5 }}>
          ...
          <div className="stat-sub">Not monitored</div>
        </div>
      ...
      <div className="server-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '24px' }}>
        <div className="stat-card">
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ textTransform: 'capitalize' }}>{stats?.weather ?? '—'}</div>
        </div>
        <div className="stat-card">
